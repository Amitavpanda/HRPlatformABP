...
} from '../../../proxy/payroll-records/models';
import { PayrollRecordService } from '../../../proxy/payroll-records/payroll-record.service';

export abstract class AbstractPayrollRecordViewService {
  protected readonly proxyService = inject(PayrollRecordService);
  ...
  data: PagedResultDto<PayrollRecordWithNavigationPropertiesDto> = {
    items: [],
    totalCount: 0,
  };

  selectionType = SelectionType;
  selected = signal<PayrollRecordWithNavigationPropertiesDto[]>([]);
  allSelected = signal(false);
  selectedCount = computed(() => this.selected().length);

  filters = {} as GetPayrollRecordsInput;
  ...
  protected bulkDeleteRequest() {
    const ids = this.selected().map(({ payrollRecord: { id } }) => id);
  ...
  delete(record: PayrollRecordWithNavigationPropertiesDto) {
    this.confirmationService
      ...
      .pipe(
        ...
        switchMap(() => this.proxyService.delete(record.payrollRecord.id)),
      )
  ...
  hookToQuery() {
    ...
    const setData = (list: PagedResultDto<PayrollRecordWithNavigationPropertiesDto>) => {
      this.data = list;
  ...
  clearFilters() {
    this.filters = {} as GetPayrollRecordsInput;
    this.list.get();
  }
  ...
  exportToExcel() {
    ...
    this.proxyService
      ...
      .subscribe(result => {
        this.abpWindowService.downloadBlob(result, 'PayrollRecord.xlsx');
      });
  }
}
